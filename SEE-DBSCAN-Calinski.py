...
# -------------------------
# SEE using DBSCAN with Calinski
# -------------------------
def SEE_dbscan(arg1):
    # Filter rows where ATC equals arg1
    subset = tidy[tidy['ATC'] == arg1].copy()
    ...
    for eps in eps_candidates:
        ...
        if len(unique_labels - {-1}) > 1:  # More than one valid cluster
            score = calinski_harabasz_score(X, candidate_labels)
            ch_scores[eps] = score

    if ch_scores:
        optimal_eps = max(ch_scores, key=ch_scores.get)  # Higher CH index is better
    else:
        optimal_eps = 0.5  # fallback if CH score could not be computed

    # Plot Calinski–Harabasz index as a function of eps
    plt.figure(figsize=(8, 5))
    plt.plot(list(ch_scores.keys()), list(ch_scores.values()), marker='o')
    plt.title("Calinski–Harabasz Analysis (DBSCAN)")
    plt.xlabel("eps")
...
print("Mann–Whitney U Test comparing KMeans vs DBSCAN (using CH index):")
