...
# -------------------------
# SEE using OPTICS with Davies–Bouldin Analysis to find optimal min_samples
# -------------------------
def SEE_optics(arg1):
    # Filter rows where ATC equals arg1
    subset = tidy[tidy['ATC'] == arg1].copy()
    ...
    # --- Davies–Bouldin Analysis using OPTICS ---
    ...
    for ms in candidate_min_samples:
        ...
        # Only compute score if more than one cluster is found (ignoring noise marked as -1)
        if len(unique_labels - {-1}) > 1:
            score = davies_bouldin_score(X_scaled, candidate_labels)
            db_scores[ms] = score
    if db_scores:
        optimal_ms = min(db_scores, key=db_scores.get)
    else:
        optimal_ms = 2
    
    plt.figure(figsize=(8,5))
    plt.plot(list(db_scores.keys()), list(db_scores.values()), marker='o')
    plt.title("OPTICS Davies–Bouldin Analysis (min_samples)")
...
print("Mann–Whitney U Test comparing KMeans vs OPTICS (using Davies–Bouldin for min_samples):")
