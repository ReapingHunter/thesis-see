...
def SEE_dbscan(arg1):
    ...
    for eps in eps_candidates:
        ...
        if len(valid_clusters) > 1:
            dunn = compute_dunn_index(X_scaled, candidate_labels)
            if dunn is not None:
                dunn_scores[eps] = dunn

    if dunn_scores:
        # For Dunn index, a higher value is better
        optimal_eps = max(dunn_scores, key=dunn_scores.get)
    else:
        optimal_eps = 0.5  # fallback if Dunn index could not be computed

    # Plot Dunn index scores as a function of eps
    plt.figure(figsize=(8, 5))
    plt.plot(list(dunn_scores.keys()), list(dunn_scores.values()), marker='o')
    plt.title("Dunn Index Analysis (DBSCAN)")
    plt.xlabel("eps")
...
print("Mann–Whitney U Test comparing KMeans vs DBSCAN (using dunn index):")
