...
# -------------------------
# SEE using DBSCAN with Dunn Index
# -------------------------

# Helper function to compute Dunn index
def compute_dunn_index(X, labels):
    # Only consider clusters not marked as noise (-1)
    ...
    for label, points in clusters.items():
        if len(points) > 1:
            # Compute pairwise Euclidean distances in the cluster
            dists = pairwise_distances(points)
            # Diameter is the maximum distance
            diameter = dists.max()
        else:
            # For a single point, we define the diameter as 0
            diameter = 0
        intra_dists.append(diameter)
    max_intra = max(intra_dists)
    
    # Compute inter-cluster distances: minimum distance between any two points from different clusters
    inter_dists = []
    unique_labels = list(unique_labels)
    for i in range(len(unique_labels)):
        for j in range(i + 1, len(unique_labels)):
            points_i = clusters[unique_labels[i]]
            points_j = clusters[unique_labels[j]]
    ...
    # To avoid division by zero if max intra-cluster distance is 0
    if max_intra == 0:
        return np.inf
...
def See_dbscan(arg1, eps_range=(0.01, 10.0), num_eps=100, min_samples=4):
    """
    Clustering using DBSCAN with dunn-based optimal eps.
    ...
    range of eps values to select the eps that yields the highest dunn score (computed on non-noise points).
    ...
    # Search for an optimal eps by iterating over a range of eps values and computing dunn scores
    eps_values = np.linspace(eps_range[0], eps_range[1], num_eps)
    dunn_scores = {}
    for eps in eps_values:
        dbscan = DBSCAN(eps=eps, min_samples=min_samples)
        labels = dbscan.fit_predict(df_ecdf_scaled)
        # Exclude noise label (-1) for dunn computation
        labels_valid = labels[labels != -1]
        if len(np.unique(labels_valid)) < 2:
            continue  # need at least 2 clusters for dunn score
        mask = labels != -1
        score = compute_dunn_index(df_ecdf_scaled[mask], labels[mask])
        dunn_scores[eps] = score
    if dunn_scores:
        best_eps = max(dunn_scores, key=dunn_scores.get)
    else:
        best_eps = eps_range[0]  # default eps if no suitable eps found

    # Plot dunn scores for DBSCAN eps values
    plt.figure(figsize=(8, 5))
    plt.plot(list(dunn_scores.keys()), list(dunn_scores.values()), marker='o')
    plt.title("DBSCAN Dunn Analysis (eps search)")
