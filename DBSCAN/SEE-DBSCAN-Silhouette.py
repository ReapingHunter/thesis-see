...
def See_dbscan(arg1, eps_range=(0.01, 10.0), num_eps=100, min_samples=4):
    """
    Clustering using DBSCAN with silhouette-based optimal eps.
    ...
    range of eps values to select the eps that yields the highest silhouette score (computed on non-noise points).
    ...
    # Search for an optimal eps by iterating over a range of eps values and computing silhouette scores
    eps_values = np.linspace(eps_range[0], eps_range[1], num_eps)
    sil_scores = {}
    for eps in eps_values:
        dbscan = DBSCAN(eps=eps, min_samples=min_samples)
        labels = dbscan.fit_predict(df_ecdf_scaled)
        # Exclude noise label (-1) for silhouette computation
        labels_valid = labels[labels != -1]
        if len(np.unique(labels_valid)) < 2:
            continue  # need at least 2 clusters for silhouette score
        mask = labels != -1
        score = silhouette_score(df_ecdf_scaled[mask], labels[mask])
        sil_scores[eps] = score
    if sil_scores:
        best_eps = max(sil_scores, key=sil_scores.get)
    else:
        best_eps = eps_range[0]  # default eps if no suitable eps found

    # Plot silhouette scores for DBSCAN eps values
    plt.figure(figsize=(8, 5))
    plt.plot(list(sil_scores.keys()), list(sil_scores.values()), marker='o')
    plt.title("DBSCAN Silhouette Analysis (eps search)")
