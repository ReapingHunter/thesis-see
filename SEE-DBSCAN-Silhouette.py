...
# -------------------------
# SEE using DBSCAN with Silhouette Analysis
# -------------------------
def SEE_dbscan(arg1):
    # Filter rows where ATC equals arg1
    subset = tidy[tidy['ATC'] == arg1].copy()
    ...
    eps_candidates = np.linspace(0.01, 10.0, 1000)
    sil_scores = {}
    for eps in eps_candidates:
        db_candidate = DBSCAN(eps=eps, min_samples=4)  # default min_samples
    ...
    if sil_scores:
        optimal_eps = max(sil_scores, key=sil_scores.get)
    else:
        optimal_eps = 0.5
