...
# -------------------------
# SEE using KMeans with Silhouette Analysis (unchanged)
# -------------------------
def SEE_kmeans(arg1):
...
# -------------------------
# SEE using OPTICS with Calinski–Harabasz Analysis to find optimal min_samples
# -------------------------
def SEE_optics(arg1):
    # Filter rows where ATC equals arg1
    subset = tidy[tidy['ATC'] == arg1].copy()
    ...
    # --- Calinski–Harabasz Analysis using OPTICS ---
    # Loop over candidate values for min_samples (e.g., 2 to 10)
    ch_scores = {}
    candidate_min_samples = range(2, 10)
    for ms in candidate_min_samples:
        ...
        # Only compute CH index if more than one cluster exists (ignoring noise labeled as -1)
        if len(unique_labels - {-1}) > 1:
            # Filter out noise for computing CH score
            valid_mask = candidate_labels != -1
            if np.sum(valid_mask) > 1:
                score = calinski_harabasz_score(X_scaled[valid_mask], candidate_labels[valid_mask])
                ch_scores[ms] = score
    if ch_scores:
        optimal_ms = max(ch_scores, key=ch_scores.get)
    else:
        optimal_ms = 2
    
    plt.figure(figsize=(8,5))
    plt.plot(list(ch_scores.keys()), list(ch_scores.values()), marker='o')
    plt.title("OPTICS Calinski–Harabasz Analysis (min_samples)")
...
print("Mann–Whitney U Test comparing KMeans vs OPTICS (using Calinski–Harabasz):")
