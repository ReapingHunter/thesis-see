...
def SEE_agglomerative(arg1):
    ...
    # --- Calinski-Harabasz Analysis using Agglomerative Clustering ---
    ...
    for n in n_candidates:
        ...
        # Compute the Calinski-Harabasz score if more than one cluster exists
        if len(set(candidate_labels)) > 1:
            score = calinski_harabasz_score(X_scaled, candidate_labels)
            calinski_scores[n] = score
    if calinski_scores:
        optimal_n = max(calinski_scores, key=calinski_scores.get)
    else:
        optimal_n = 2
    
    plt.figure(figsize=(8,5))
    plt.plot(list(calinski_scores.keys()), list(calinski_scores.values()), marker='o')
    plt.title("Agglomerative Clustering Calinski-Harabasz Analysis")
    plt.xlabel("Number of clusters")
