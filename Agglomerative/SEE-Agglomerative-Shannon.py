...
#from sklearn.metrics import davies_bouldin_score  # No longer needed
...
# -------------------------
# Helper Function: Compute Shannon Entropy of clustering labels
# -------------------------
def clustering_entropy(labels):
    """
    Compute Shannon entropy for the clustering labels.
    
    Parameters:
        labels: array-like, cluster labels for each sample.
    
    Returns:
        entropy: float (higher entropy indicates a more balanced distribution).
    """
    # Here we assume that noise labels are not present (all labels are valid)
...
# -------------------------
# SEE using Agglomerative Clustering with Shannon Entropy Analysis
# -------------------------
def SEE_agglomerative(arg1):
    # (Function name remains, but now it uses Agglomerative Clustering instead of DBSCAN.)
    ...
    # --- Shannon Entropy Analysis using Agglomerative Clustering ---
    ...
    for n in n_candidates:
        ...
        # Compute Shannon entropy of the cluster distribution
        score = clustering_entropy(candidate_labels)
        entropy_scores[n] = score
    if entropy_scores:
        optimal_n = min(entropy_scores, key=entropy_scores.get)
    else:
        optimal_n = 2
    
    plt.figure(figsize=(8,5))
    plt.plot(list(entropy_scores.keys()), list(entropy_scores.values()), marker='o')
    plt.title("Agglomerative Clustering Shannon Entropy Analysis")
    plt.xlabel("Number of clusters")
...
final_ag, details_ag = SEE_agglomerative("medA")  # Now using Agglomerative Clustering with Shannon entropy selection
...
print("Mann–Whitney U Test comparing KMeans vs Agglomerative Clustering (using Shannon entropy):")
