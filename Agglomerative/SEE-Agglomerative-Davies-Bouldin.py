...
def SEE_agglomerative(arg1):
    ...
    # --- Davies-Bouldin Analysis using Agglomerative Clustering ---
    ...
    for n in n_candidates:
        ...
        # Compute the Davies-Bouldin score if more than one cluster exists
        if len(set(candidate_labels)) > 1:
            score = davies_bouldin_score(X_scaled, candidate_labels)
            davies_bouldin_scores[n] = score
    if davies_bouldin_scores:
        optimal_n = min(davies_bouldin_scores, key=davies_bouldin_scores.get)
    else:
        optimal_n = 2
    
    plt.figure(figsize=(8,5))
    plt.plot(list(davies_bouldin_scores.keys()), list(davies_bouldin_scores.values()), marker='o')
    plt.title("Agglomerative Clustering Davies-Bouldin Analysis")
    plt.xlabel("Number of clusters")
...
print("Mann–Whitney U Test comparing KMeans vs Agglomerative Clustering (using DB score):")
